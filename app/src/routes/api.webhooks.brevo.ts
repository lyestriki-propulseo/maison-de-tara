...
async function handleReservation(db: Db, id: string) {
  const { data: claimed, error: claimError } = await db
    ...
    .maybeSingle()
  if (claimError) {
    console.error('[webhook:reservation] claim failed:', claimError)
    return
  }
  if (!claimed) return
  ...
  try {
    ...
    console.error('[webhook:reservation] handler failed after claim:', err)
  }
}
...
async function handleNewsletter(db: Db, id: string) {
  const { data: claimed, error: claimError } = await db
    ...
    .maybeSingle()
  if (claimError) {
    console.error('[webhook:newsletter] claim failed:', claimError)
    return
  }
  if (!claimed) return
  ...
  try {
    ...
    console.error('[webhook:newsletter] handler failed after claim:', err)
  }
}
...
async function handleRequest(db: Db, id: string) {
  const { data: claimed, error: claimError } = await db
    ...
    .maybeSingle()
  if (claimError) {
    console.error('[webhook:request] claim failed:', claimError)
    return
  }
  if (!claimed) return
  ...
  try {
    ...
    console.error('[webhook:request] handler failed after claim:', err)
  }
}
